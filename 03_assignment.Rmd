...
#  head(100) %>% 
  ...
  summarize(n_flights = n(),
            distinct_dest = distinct(dest), #need to fix this
...
```{sql connection=}
SELECT `origin`, `year`, `month`, COUNT(*) AS `n_flights`, select(`dest`) AS `distinct_dest`, AVG(`air_time`) AS `avg_length`, AVG(`distance`) AS `avg_distance`, AVG(`arr_delay` > 20.0) AS `prop_late_over20`
FROM `flights`
