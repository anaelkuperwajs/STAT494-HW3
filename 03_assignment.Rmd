...
We can see in all three graphs that the intercept is 5.665, meaning that is the average predicted log_price when the random forest is applied to all the training data. The final predicted outcome is slightly different for each observation, but between 5.83-6.15. The contribution of the variables to each prediction, however, varies greatly between each observation. The model isn't additive, so the order of variables matters to these plots.

For the first observation, sqft_living = 3790 contributes the most.
...
The SHAP graph takes the average contributions of variables from a variety of permutations of break-down plots. For the second two observations, the variable that contributes the most is the same. For the first observation, however, the variable that contributes the most in the SHAP graph is the second in the break-down plot, showing that taking the average is important to get an accurate reading on contribution. Furthermore, the contribution to the predicted outcome is usually mostly built from a few variables.
...
The first and third observations have a reasonable explanation fit, while the second observation has an extremely low explanation fit. This makes me belive that the second observation is not as reliable as the other two. That being said, the original prediction and the prediction from the local model are all very similar. About 0.11 difference for the first and third observation and about 0.15 difference for the second observation. The plots show the variables that have the most weight in the local model, and the most important variable usually does correspond to the first or second most important variable in the SHAP graph and break-down plots.
...
ANSWER
tools:
  global: Model performance, Variable importance, Ceteris-paribus profiles, Partial dependence plots
  local: CP Profile, breakdown plots, SHAP, LIME
...
SELECT *
FROM (SELECT `origin`, `year`, `month`, `n_flights`, `distinct_dest`, `avg_air_time`, `avg_distance`, `prop_late_over20`, `name`, `lat`, `lon`, `alt`, `tz`, `dst`, `city`, `country`
FROM (SELECT `origin`, `year`, `month`, COUNT(*) AS `n_flights`, COUNT(DISTINCT `dest`) AS `distinct_dest`, AVG(`air_time`) AS `avg_air_time`, AVG(`distance`) AS `avg_distance`, AVG(`arr_delay` > 20.0) AS `prop_late_over20`
FROM (SELECT *
FROM `flights`
LIMIT 200000) `q01`
WHERE (`year` = 2010.0)
...
flights_many_df <- collect(flights_many)
...
flights_many_table <- flights_many_df %>% 
  select(name, prop_late_over20, n_flights) %>% 
  arrange(desc(prop_late_over20)) %>% 
  head(6)

flights_many_table

knitr::kable(flights_many_table, col.names = gsub("[.]", " ", names(flights)))
table(flights_many_table)
```
...
```{sql connection = con_air}
SELECT `month`, COUNT(*) AS `n_flights`
FROM `flights`
WHERE (`year` = 2010.0)
GROUP BY `month`
...
```{r}
flights_months_over_time <- tbl(con_air, "flights") %>% 
  group_by(month, year) %>% 
  summarize(n_flights = n()) %>% 
  arrange(desc(n_flights))

show_query(flights_months_over_time)

flights_months_over_time_df <- collect(flights_months_over_time)
flights_months_over_time_df
```
...
```{r}
flights_largest_month <- flights_months_over_time %>%
  filter()

flights_months_over_time %>% 
  ggplot(aes(x = month, y = n_flights)) +
  geom_col(fill = "lightblue") +
  labs(x = "Month",
       y = "Number of Flights",
